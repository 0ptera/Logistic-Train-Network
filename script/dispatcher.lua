...
function ProcessRequest(reqIndex, request)
  ...
  if not providers or #providers < 1 then
    if requestStation.no_warnings == false and message_level >= 1 then printmsg({"ltn-message.no-provider-found", to_gps, "[img=" .. itype .. "/" .. iname .. "]", localname, to_network_id_string, "[img=" .. itype .. "/" .. iname .. "]"}, requestForce, true) end
