  ...
  if stop then
    ...
    if stop.errorCode == 0 then
      if stop.isDepot then
        ...
        global.Dispatcher.availableTrains[train.id] = {train = train, surface = loco.surface, force = trainForce, network_id = stop.network_id, capacity = capacity, fluid_capacity = fluid_capacity}
